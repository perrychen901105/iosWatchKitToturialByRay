...
private let kAppGroupIdentifier = "group.com.aodshop.souschef.documents"
private let kInitialRecipesCopiedKey = "com.rw.souschef.recipesCopied"

private let kRemoteRecipesURLString = "https://raw.githubusercontent.com/perrychen901105/recipes/master/Recipes.json"
...
public class RecipeStore {
  ...
  public init() {
    // 1
    // create an instance of NSUserDefaults that's instantiated with your registered app group;
  ...
  public func refresh(#completion:((recipes: [Recipe], error: NSError?) -> Void)?) {
    ...
    if let remoteRecipesURL = NSURL(string: kRemoteRecipesURLString) {
      let task = session.dataTaskWithURL(remoteRecipesURL, completionHandler: {
        (data: NSData?,
